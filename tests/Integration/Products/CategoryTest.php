...
class CategoryTest extends \PHPUnit_Framework_TestCase
{
	...
	{
		$code = 'CatTest001';
		$resource = $this->request->get($code);
		$this->assertInstanceOf('\GlauberPortella\SkyHub\Resource\Category', $resource);

		$this->assertEquals($code, $resource->code);
		$this->assertEquals('Test category skyhub-php library', $resource->nome);

		$resource->name = 'Test category skyhub-php library updated';
		$this->request->put($resource);

		$updated = $this->request->get($code);
		$this->assertEquals($code, $resource->code);
		$this->assertEquals('Test category skyhub-php library updated', $resource->nome);
